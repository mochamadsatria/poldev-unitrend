...
export default function Home() {
  const positiveBrandTrendSorted = Object.keys(
    trendsData["Keuangan"]["brand_data"]
  )
    .filter((brand) => trendsData["Keuangan"]["brand_data"][brand] > 0.0)
    .sort(
      (a, b) =>
        -(
          trendsData["Keuangan"]["brand_data"][a] -
          trendsData["Keuangan"]["brand_data"][b]
        )
    );

  const positiveBrandTrendSortedData = positiveBrandTrendSorted.map(
    (brand) => trendsData["Keuangan"]["brand_data"][brand]
  );

  const negativeBrandTrendSorted = Object.keys(
    trendsData["Keuangan"]["brand_data"]
  )
    .filter((brand) => trendsData["Keuangan"]["brand_data"][brand] < 0.0)
    .sort(
  ...
  const positiveWordTrendSorted = Object.keys(
    trendsData["Keuangan"]["word_data"]
  )
    .filter((word) => trendsData["Keuangan"]["word_data"][word] > 0.0)
    .sort(
      (a, b) =>
        -(
          trendsData["Keuangan"]["word_data"][a] -
          trendsData["Keuangan"]["word_data"][b]
        )
    );

  const positiveWordTrendSortedData = positiveWordTrendSorted.map(
    (word) => trendsData["Keuangan"]["word_data"][word]
  );

  const negativeWordTrendSorted = Object.keys(
    trendsData["Keuangan"]["word_data"]
  )
    .filter((word) => trendsData["Keuangan"]["word_data"][word] < 0.0)
    .sort(
  ...
  return (
    ...
    <div className="min-h-screen flex items-center bg-[#222]">
      <div className="flex-1 max-w-7xl mx-auto p-10">
        <section>
          <h1 className="text-white text-[2rem] font-bold">Keuangan</h1>
        ...
        <section>
          ...
          <LineCharts
            trendsData2021={Object.values(
              trendsData["Keuangan"]["biannually_brand_trends_mean"]["2021"]
            )}
            trendsData2022={Object.values(
              trendsData["Keuangan"]["biannually_brand_trends_mean"]["2022"]
            )}
          />
        </section>
        <section className="columns-2">
          <div className="flex flex-col items-center justify-center border border-solid border-white p-8">
            ...
            <h2 className="text-white text-[2rem] font-bold">
                {trendsData["Keuangan"]["brand_trends_mean_change"].toFixed(1)}%
            </h2>
          </div>
          <div className="flex flex-col items-center justify-center border border-solid border-white p-8">
            <p className="text-white text-[1.1rem]">
            ...
            <h2 className="text-white text-[2rem] font-bold">
                {trendsData["Keuangan"]["word_trends_mean_change"].toFixed(1)}%
            </h2>
          </div>
        </section>
        <section>
